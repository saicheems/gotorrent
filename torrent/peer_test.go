...
func TestSendMessage(t *testing.T) {
	...
	testSendMessage(t, Choke{}, []byte{1, 0, 0, 0, 0})
	testSendMessage(t, Unchoke{}, []byte{1, 0, 0, 0, 1})
	testSendMessage(t, Interested{}, []byte{1, 0, 0, 0, 2})
	testSendMessage(t, NotInterested{}, []byte{1, 0, 0, 0, 3})
	testSendMessage(t, Have{PieceIndex: 123456}, []byte{5, 0, 0, 0, 4, 0x40, 0xe2, 0x01, 0x00})
	testSendMessage(t, Bitfield{Data: []byte{1, 2, 3, 4}}, []byte{5, 0, 0, 0, 5, 1, 2, 3, 4})
	testSendMessage(t, Request{Index: 1, Begin: 2, Length: 3}, []byte{13, 0, 0, 0, 6, 1, 0, 0, 0, 2, 0, 0, 0, 3, 0, 0, 0})
	testSendMessage(t, Piece{Index: 1, Begin: 2, Block: []byte{3, 4, 5}}, []byte{12, 0, 0, 0, 7, 1, 0, 0, 0, 2, 0, 0, 0, 3, 4, 5})
	testSendMessage(t, Cancel{Index: 1, Begin: 2, Length: 3}, []byte{13, 0, 0, 0, 8, 1, 0, 0, 0, 2, 0, 0, 0, 3, 0, 0, 0})
}
...
func TestReceiveMessage(t *testing.T) {
	...
	res = ParseMessage([]byte{1, 0, 0, 0, 0})
	assert.Equal(res.(Choke), Choke{})
	res = ParseMessage([]byte{1, 0, 0, 0, 1})
	assert.Equal(res.(Unchoke), Unchoke{})
	res = ParseMessage([]byte{1, 0, 0, 0, 2})
	assert.Equal(res.(Interested), Interested{})
	res = ParseMessage([]byte{1, 0, 0, 0, 3})
	assert.Equal(res.(NotInterested), NotInterested{})
	res = ParseMessage([]byte{5, 0, 0, 0, 4, 0x40, 0xe2, 0x01, 0x00})
	assert.Equal(res.(Have), Have{PieceIndex: 123456})
	res = ParseMessage([]byte{5, 0, 0, 0, 5, 1, 2, 3, 4})
	assert.Equal(res.(Bitfield), Bitfield{Data: []byte{1, 2, 3, 4}})
	res = ParseMessage([]byte{13, 0, 0, 0, 6, 1, 0, 0, 0, 2, 0, 0, 0, 3, 0, 0, 0})
	assert.Equal(res.(Request), Request{Index: 1, Begin: 2, Length: 3})
	res = ParseMessage([]byte{12, 0, 0, 0, 7, 1, 0, 0, 0, 2, 0, 0, 0, 3, 4, 5})
	assert.Equal(res.(Piece), Piece{Index: 1, Begin: 2, Block: []byte{3, 4, 5}})
	res = ParseMessage([]byte{13, 0, 0, 0, 8, 1, 0, 0, 0, 2, 0, 0, 0, 3, 0, 0, 0})
